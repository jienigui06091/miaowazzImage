...
def _load_settings() -> LoadedSettings:
    ...
    if _is_invalid_auth_key(auth_key):
        raise ValueError(
            "❌ auth-key 未设置！\n"
            "请在环境变量 MIAOWAZZIMAGE_AUTH_KEY 中设置，或者在 config.json 中填写 auth-key。"
        )
...
class ConfigStore:
    def __init__(self, path: Path):
        ...
        if _is_invalid_auth_key(self.auth_key):
            raise ValueError(
                ...
                "   MIAOWAZZIMAGE_AUTH_KEY = your_real_auth_key\n"
    ...
    def auth_key(self) -> str:
        return _normalize_auth_key(_env_value("MIAOWAZZIMAGE_AUTH_KEY", "CHATGPT2API_AUTH_KEY") or self.data.get("auth-key"))
    ...
    def base_url(self) -> str:
        return str(
            _env_value("MIAOWAZZIMAGE_BASE_URL", "CHATGPT2API_BASE_URL")
